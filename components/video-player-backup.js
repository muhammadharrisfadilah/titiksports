    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, async (event, data) => {
        ...
        if (data.details === Hls.ErrorDetails.BUFFER_STALLED_ERROR) {
          ...
          console.warn(`⏸️ Buffer stall #${stallRetryCountRef.current}/${MAX_STALL_RETRIES} (consecutive: ${stallConsecutiveRef.current})`);
          
          if (stallRetryCountRef.current >= MAX_STALL_RETRIES) {
            console.error('❌ Buffer stall exhausted all retries');
            stallRetryCountRef.current = 0;
            stallConsecutiveRef.current = 0;
            failedLinksRef.current.add(linkId);
            tryAlternativeLink();
          ...
          if (isRecoveringRef.current) {
            console.log('⏳ Already recovering from stall, waiting...');
            return;
          }
          ...
          const recoverySteps = [
            ...
            () => {
              console.log('📢 Recovery step 1: Attempting normal resume');
            ...
            // Step 2: Drop bitrate
            () => {
              console.log('📢 Recovery step 2: Dropping bitrate');
              if (hls && hls.levels.length > 0) {
                const currentLevel = hls.currentLevel;
                if (currentLevel > 0) {
                  hls.currentLevel = Math.max(0, currentLevel - 1);
                  console.log(`⬇️ Switched to lower bitrate level ${hls.currentLevel}`);
                }
              }
              if (video && video.paused && !video.ended) {
                video.play().catch(e => console.log('Resume failed:', e.message));
            ...
            () => {
              console.log('📢 Recovery step 3: Clearing buffer and restarting');
              if (hls) {
                hls.stopLoad();
                setTimeout(() => {
            ...
            () => {
              console.log('📢 Recovery step 4: Forcing lowest bitrate');
              if (hls && hls.levels.length > 0) {
                hls.currentLevel = 0;
                if (video && video.paused && !video.ended) {
                  video.play().catch(e => console.log('Resume failed:', e.message));
      ...
      hls.on(Hls.Events.LEVEL_LOADING_ERROR, (event, data) => {
        console.warn(`⚠️ Level loading error: level ${data.context.level}`, data.error);
        
        // Try lower level if available
        if (hls.levels.length > 1 && hls.currentLevel > 0) {
          console.log('📉 Falling back to lower quality level');
